...
def register_chief_route():
    ...
    if request.method == 'POST':
        ...
        role = request.form.get('role', 'chief')
        group = request.form.get('group', 'Без группы').strip()
        start_date_str = request.form.get('start_date', '2021-09-01')
        end_date_str = request.form.get('end_date', '2025-08-31')

        # Валидация ФИО
        if not re.match(r'^[А-Яа-яЁё\s-]+$', full_name):
            message = 'ФИО должно содержать только буквы русского алфавита.'
            message_type = 'danger'
            return render_template('authorization/registration/register_chief.html', message=message, message_type=message_type, full_name=full_name, email=email, institute=institute, role=role, group=group, start_date=start_date_str, end_date=end_date_str)
        ...
        return redirect(url_for('confirm_email_register_chief'))

    return render_template('authorization/registration/register_chief.html', message=message, message_type=message_type)
