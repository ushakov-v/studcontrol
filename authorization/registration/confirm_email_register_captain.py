...
def confirm_email_register_captain_route():
    if request.method == 'POST':
        ...
        if temp_user_id:
            ...
            if temp_user and temp_user.email_confirmation_code == confirmation_code:
                ...
                if temp_users:
                    ...
                    # Создание запросов на одобрение для старосты
                    if user.role == 'captain':
                        chiefs = User.query.filter_by(role='chief', institute=user.institute).all()
                        ...
                        if chiefs:
                            for chief in chiefs:
                                request_entry = Request(
                                    ...
                                    captain_id=chief.id,  # Заведующие
                                ...
                                message = Message(
                                    sender_id=user.id,
                                    recipient_id=chief.id,
                                    subject='Запрос на просмотр группы',
                                    body=f'Пользователь {user.full_name} зарегистрировался в системе как староста и запрашивает доступ к системе.',
                                    timestamp=datetime.utcnow(),
                                    request_id=request_entry.id  # Устанавливаем request_id
                                )
                                db.session.add(message)

                                # Отправляем email заведующему
                                send_email_requests(
                                    'Запрос на просмотр группы',
                                    chief.email,
                                    f'Пользователь {user.full_name} зарегистрировался в системе как староста и запрашивает доступ к системе.',
                        ...
                        if admin:
                            request_entry = Request(
                                student_id=user.id,  # Используем постоянного пользователя
                                captain_id=admin.id,  # Администратор
                                institute=user.institute
                            )
                            db.session.add(request_entry)
                            db.session.commit()
        ...
        return render_template('authorization/registration/confirm_email_register_captain.html', message=message)

    return render_template('authorization/registration/confirm_email_register_captain.html')
